...
async function renderPublicProfile(container, ctx) {
  ...
  const [profile, following, followerCount] = await Promise.all([
...
async function renderProfile(container, ctx) {
  const p = ctx.profile;
  const followerCount = await DB.countFollowers(ctx.user.id).catch(() => 0);
  ...
  container.innerHTML = `
    ...
    <div class="profile-menu">
      ...
      <button class="profile-menu-item" data-goto="about">ℹ️ À propos de Together We Can</button>
      ${!p.is_admin && ctx.user.email === "jephbesaus07@gmail.com" ? `<button class="profile-menu-item" id="btn-unlock-admin">🔑 Activer le panel admin</button>` : ""}
    </div>
  `;
  ...
  if (unlockBtn) {
    unlockBtn.addEventListener("click", async () => {
      ...
      if (!code) return;
      try {
        const ok = await DB.selfActivateAdmin(code);
        if (ok) {
          alert("✅ Panel administrateur activé !");
          ctx.profile.is_admin = true;
          renderMenu();
          renderProfile(container, ctx);
        } else {
          alert("❌ Code incorrect ou compte non autorisé.");
        }
      } catch (err) {
        alert("❌ " + err.message);
      }
...
async function renderSecurity(container, ctx) {
  container.innerHTML = `
    <form id="pwd-form" class="inline-form card-form">
      ...
      <label class="field"><span>Ancien mot de passe</span><input type="password" name="old_password" required /></label>
  ...
  container.querySelector("#pwd-form").addEventListener("submit", async (e) => {
    ...
    try {
      const { error: authErr } = await supabaseClient.auth.signInWithPassword({
        ...
        password: fd.get("old_password"),
      });
      if (authErr) throw new Error("Ancien mot de passe incorrect.");
