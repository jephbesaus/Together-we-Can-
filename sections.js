...
async function renderMarketing(container, ctx) {
  let mine = [];
  try {
    mine = await DB.myMarketingRequests(ctx.user.id);
  } catch (_) {}
  ...
  container.innerHTML = `
    ...
    <form id="marketing-form" class="inline-form card-form">
      <label class="field"><span>Décrivez votre besoin</span>
        <textarea name="need" rows="3" placeholder="Ex : j'ai besoin de faire une publicité concernant mes produits" required></textarea>
      </label>
      <label class="field"><span>Vitesse souhaitée</span>
        <select name="speed" required>
          <option value="accelere">Marketing accéléré</option>
          <option value="simple">Marketing simple</option>
          <option value="lent">Marketing lent</option>
        </select>
      </label>
      <label class="field"><span>Quel type de marketing ?</span>
        <select name="goal" required>
          <option value="vente">Vente de mes produits</option>
          <option value="groupe">Appel aux gens pour rejoindre mon groupe</option>
          <option value="formations">Recherche de personnes pour mes formations</option>
    ...
    <div class="card-list">
      ${
        mine.length
          ? mine.map((r) => `<article class="content-card"><div class="content-card-head"><h3>${esc(r.goal)}</h3>${statusBadge(r.status)}</div><p class="content-card-meta">${esc(r.need)}</p><p class="content-card-meta">${formatDate(r.created_at)}</p></article>`).join("")
          : `<p class="section-loading">Aucune demande envoyée.</p>`
      }
    </div>
  `;

  container.querySelector("#marketing-form").addEventListener("submit", async (e) => {
    e.preventDefault();
    const form = e.target;
    const status = form.querySelector(".form-status");
    const fd = new FormData(form);
    try {
      await DB.createMarketingRequest({
        user_id: ctx.user.id,
        need: fd.get("need"),
        speed: fd.get("speed"),
        goal: fd.get("goal"),
      ...
      await DB.sendMessage(
        ctx.user.id,
        `Bonjour, je m'appelle Jeph Besaus. Je reste à l'écoute de votre budget, nous pouvons en parler afin de bien confirmer votre marketing.`,
        true
      );
      status.textContent = "✅ Demande envoyée ! Consultez « Messages » pour discuter du budget avec l'administration.";
      status.className = "form-status success";
      status.classList.remove("hidden");
      form.reset();
      setTimeout(() => renderMarketing(container, ctx), 1200);
    } catch (err) {
      status.textContent = "❌ " + err.message;
      status.className = "form-status error";
      status.classList.remove("hidden");
...
async function renderMarketplace(container, ctx) {
  ...
  container.innerHTML = `
    <details class="admin-add">
      ...
      <form class="inline-form" id="product-form">
        ...
        <input type="text" name="location" placeholder="Localisation (ville, quartier...)" required />
    ...
    <div class="card-list">
      ${
        ...
        <article class="content-card" data-id="${p.id}">
          ...
          <div class="content-card-head"><h3>${esc(p.title)}</h3><span class="badge badge-gold">${p.price} ${esc(p.currency || "FC")}</span></div>
          <p class="content-card-body">${esc(p.description || "")}</p>
          <p class="content-card-meta">Vendeur : ${esc(p.profiles?.full_name || "Membre")}${p.location ? ` · 📍 ${esc(p.location)}` : ""}${p.is_sold ? " · ✅ Vendu" : ""}</p>
          ${p.seller_id === ctx.user.id && !p.is_sold ? `
          <div class="content-card-actions">
            <button class="btn-secondary" data-sold="${p.id}">✅ Marquer vendu</button>
            <button class="btn-secondary" data-delproduct="${p.id}">🗑️ Supprimer</button>
          </div>` : ""}
        </article>`
              )
              .join("")
  ...
  container.querySelectorAll("[data-sold]").forEach((btn) =>
    btn.addEventListener("click", async () => {
      await DB.markProductSold(btn.dataset.sold);
      renderMarketplace(container, ctx);
    })
  );
  container.querySelectorAll("[data-delproduct]").forEach((btn) =>
    btn.addEventListener("click", async () => {
      if (confirm("Supprimer ce produit ?")) {
        await DB.deleteProduct(btn.dataset.delproduct);
        renderMarketplace(container, ctx);
      }
    })
  );
  ...
  form.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      status.textContent = "✅ Produit publié ! Il apparaît aussi dans Découvrir.";
      status.className = "form-status success";
      status.classList.remove("hidden");
      setTimeout(() => renderMarketplace(container, ctx), 900);
    } catch (err) {
      status.textContent = "❌ " + err.message;
      status.className = "form-status error";
      status.classList.remove("hidden");
