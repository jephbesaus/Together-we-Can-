...
async function renderAdminPosts(body) {
  ...
  const posts = await DB.adminListPendingPosts();
  if (!posts.length) {
    body.innerHTML = `<div class="empty-state"><div class="placeholder-badge">✅</div><h2>Aucune publication en attente</h2><p>Tout est à jour.</p></div>`;
    return;
  }
  body.innerHTML = `<div class="card-list">${posts
    .map(
      (p) => `<article class="content-card" data-id="${p.id}">
      <div class="content-card-head"><h3>${esc(p.profiles?.full_name || "Membre")}</h3>${statusBadge("en_attente")}</div>
      <p class="content-card-body">${esc(p.content)}</p>
      <div class="content-card-actions">
        <button class="btn-secondary" data-approve="${p.id}">✅ Publier</button>
        <button class="btn-secondary" data-reject="${p.id}">❌ Refuser</button>
      </div>
    </article>`
    )
    .join("")}</div>`;
  ...
  body.querySelectorAll("[data-approve]").forEach((btn) =>
    btn.addEventListener("click", async () => {
      await DB.adminApprovePost(btn.dataset.approve);
      renderAdminPosts(body);
    })
  );
  body.querySelectorAll("[data-reject]").forEach((btn) =>
    btn.addEventListener("click", async () => {
      if (confirm("Refuser et supprimer cette publication ?")) {
        await DB.adminRejectPost(btn.dataset.reject);
        renderAdminPosts(body);
      }
    })
  );
...
async function renderAdminUsers(body) {
  ...
  body.innerHTML = `<div class="card-list">${users
    .map(
      (u) => `<article class="content-card">
      <div class="content-card-head"><h3>${esc(u.full_name || "Membre")}</h3>${u.is_admin ? '<span class="badge badge-gold">Admin</span>' : ""}</div>
      <p class="content-card-meta">${esc(u.phone || "")} · ${u.points || 0} points · Code: ${u.referral_code} · Inscrit le ${formatDate(u.created_at)}</p>
    </article>`
    )
    .join("")}</div>`;
}
...
async function renderAdminPoints(body) {
  ...
  body.querySelectorAll("[data-validate]").forEach((btn) =>
    btn.addEventListener("click", async () => {
      ...
      await DB.adminValidateReward(req);
